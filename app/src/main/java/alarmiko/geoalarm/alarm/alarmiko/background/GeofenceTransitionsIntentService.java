...
import alarmiko.geoalarm.alarm.alarmiko.misc.DaysOfWeek;
...
public class GeofenceTransitionsIntentService extends IntentService {
    ...
    private static final String ACTION_SCHEDULE_ALARM = "alarmiko.geoalarm.alarm.alarmiko.background.action.SCHEDULE_ALARM";
    private static final String ACTION_CANCEL_ALARM = "alarmiko.geoalarm.alarm.alarmiko.background.action.CANCEL_ALARM";
    public static final String ACTION_SCHEDULE_ALARM_FROM_WIDGET = "alarmiko.geoalarm.alarm.alarmiko.background.action.SCHEDULE_ALARM_FROM_WIDGET";
    public static final String ACTION_CANCEL_ALARM_FROM_WIDGET = "alarmiko.geoalarm.alarm.alarmiko.background.action.CANCEL_ALARM_FROM_WIDGET";


    public static final String EXTRA_ALARMS = "alarmiko.geoalarm.alarm.alarmiko.background.extra.ALARMS";
    ...
    public static void scheduleGeoAlarmFromWidget(Context context, ArrayList<Alarm> alarms) {
        Intent intent = new Intent(context.getApplicationContext(), GeofenceTransitionsIntentService.class);
        intent.setAction(ACTION_SCHEDULE_ALARM_FROM_WIDGET);
        intent.putParcelableArrayListExtra(EXTRA_ALARMS, alarms);
        context.startService(intent);
    }

    public static void cancelGeoAlarmFromWidget(Context context, ArrayList<Alarm> alarms) {
        Intent intent = new Intent(context.getApplicationContext(), GeofenceTransitionsIntentService.class);
        intent.setAction(ACTION_CANCEL_ALARM_FROM_WIDGET);
        intent.putParcelableArrayListExtra(EXTRA_ALARMS, alarms);
        context.startService(intent);
    }
    ...
    protected void onHandleIntent(Intent intent) {
        if (intent != null) {
            ...
            if (ACTION_SCHEDULE_ALARM.equals(mAction)) {
                handleScheduleAlarm(intent, false);
            } else if (ACTION_CANCEL_ALARM.equals(mAction)) {
                handleCancelAlarm(intent, false);
            } else if (ACTION_SCHEDULE_ALARM_FROM_WIDGET.equals(mAction)) {
                handleScheduleAlarm(intent, true);
            } else if (ACTION_CANCEL_ALARM_FROM_WIDGET.equals(mAction)) {
                handleCancelAlarm(intent, true);
            }
        }
    }

    private void handleCancelAlarm(Intent intent, boolean fromWidget) {
        mAlarms = intent.getParcelableArrayListExtra(EXTRA_ALARMS);
        Log.d(TAG, "handleCancelAlarm: " + mAlarms);
        ...
        if (fromWidget) {
            AlarmController controller = new AlarmController(getApplicationContext(), null);
    ...
    private void handleScheduleAlarm(Intent intent, boolean fromWidget) {
        mAlarms = intent.getParcelableArrayListExtra(EXTRA_ALARMS);
        Log.d(TAG, "handleScheduleAlarm: " + mAlarms);
...
//            fences.add(basicFence);
    ...
    public void onConnected() {
        ...
        if (ACTION_SCHEDULE_ALARM.equals(mAction) || ACTION_SCHEDULE_ALARM_FROM_WIDGET.equals(mAction)) {
            for (Alarm alarm : mAlarms) {
        ...
        } else if (ACTION_CANCEL_ALARM.equals(mAction) || ACTION_CANCEL_ALARM_FROM_WIDGET.equals(mAction)) {
            for (Alarm alarm : mAlarms) {
    ...
    public void onDestroy() {
        super.onDestroy();
        Alarmiko.updateWidget(getApplicationContext());
        disconnectGoogleClient();
    }
}
