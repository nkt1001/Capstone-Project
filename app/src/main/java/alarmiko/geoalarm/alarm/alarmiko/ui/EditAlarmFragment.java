...
public class EditAlarmFragment extends BaseFragment implements OnMapReadyCallback, GoogleMap.OnCameraIdleListener,
    ...
    private static final String ARG_INSTANT_SAVE = "alarmiko.EditAlarmFragment.ARG_INSTANT_SAVE";
    ...
    @BindView(R.id.alarm_editor_layout)
    LockableScrollView mScrollLayout;
    ...
    public static EditAlarmFragment newInstance(Alarm alarm, boolean instantSave) {
        EditAlarmFragment fragment = new EditAlarmFragment();
        Bundle args = new Bundle();
        args.putParcelable(ARG_SECTION_NUMBER, alarm);
        args.putBoolean(ARG_INSTANT_SAVE, instantSave);
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mInstantSave = getArguments().getBoolean(ARG_INSTANT_SAVE);
...
//                        Log.d(TAG, "Selected ringtone: " + ringtoneUri.toString());
    ...
    private void bindLabel() {
        String label = mAlarm.label().isEmpty() ? getString(R.string.label) : mAlarm.label();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mExpandableLayout.setOnExpansionUpdateListener(new ExpandableLayout.OnExpansionUpdateListener() {
            ...
            public void onExpansionUpdate(float expansionFraction, int state) {
                if (ExpandableLayout.State.COLLAPSED == state) {
                    if (mScrollLayout == null) {
                        return;
                    }
                    mScrollLayout.setScrollingEnabled(true);
                } else if (ExpandableLayout.State.EXPANDED == state) {
                    if (mScrollLayout == null) {
                        return;
                    }
                    mScrollLayout.setScrollingEnabled(false);
                    mScrollLayout.smoothScrollTo(0, (int) mTvRadius.getY());
                }
    ...
    final void persistUpdatedAlarm(Alarm newAlarm, boolean showSnackbar) {
        ...
        if (mInstantSave || newAlarm.isEnabled()) {

            if (!mInstantSave) {
                mInstantSave = true;
                mListener.getTableUpdater().asyncInsert(newAlarm);
            } else {
    ...
    void onBtnOkClicked() {
        if (!mInstantSave) {
            mListener.getTableUpdater().asyncInsert(mAlarm);
            mInstantSave = true;
        }
